...
const PlayerBar = ({ track }: PlayerBarProps) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-20 glass border-t border-white/5 backdrop-blur-md px-4 py-3 transition-all duration-300 hover:border-neon/20">
      <div className="flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="flex items-center w-1/4">
          <HoverCard>
            <HoverCardTrigger>
              <img 
                ...
                className="h-12 w-12 rounded-lg mr-3 object-cover transition-all duration-300 hover:shadow-neon hover:scale-105" 
              />
            </HoverCardTrigger>
            <HoverCardContent className="glass-dark border-neon/20 shadow-neon-sm p-2 rounded-lg">
              <img 
                src={currentTrack.albumArt} 
                alt={`${currentTrack.title} album art`} 
                className="w-40 h-40 rounded-md object-cover mb-2 shadow-neon-sm" 
              />
          ...
          <div className="truncate mr-4">
            <p className="text-sm font-medium truncate animate-fade-in">{currentTrack.title}</p>
          ...
          <Popover>
            <PopoverTrigger asChild>
              ...
              >
                <Heart size={18} fill={isLiked ? "#009eff" : "transparent"} className="transition-all duration-300 hover:scale-110" />
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-52 glass-dark border-neon shadow-neon-sm p-2 rounded-md animate-fade-in">
        ...
        <div className="flex flex-col items-center w-2/4">
          <div className="flex items-center justify-center space-x-4">
            <button 
              className="text-gray-400 hover:text-neon transition-all duration-300 hover:scale-110"
              aria-label="Previous track"
            >
              <SkipBack size={20} />
            </button>
            <button 
              className="bg-neon rounded-full p-2 text-black hover:bg-opacity-80 transition-all duration-300 hover:shadow-neon transform hover:scale-105"
          ...
          <div className="w-full flex items-center space-x-3 mt-2">
            ...
            <div className="relative flex-1 h-1 bg-gray-700 rounded-full overflow-hidden group">
              <div 
                className="absolute top-0 left-0 h-full bg-neon rounded-full transition-all duration-300"
                style={{ width: `${progress}%` }}
              />
              <div className="absolute top-0 left-0 h-full w-full bg-gradient-to-r from-neon/40 to-neon/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              <div 
                className="absolute top-0 left-0 h-full w-2 bg-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform -translate-y-1/2"
                style={{ 
